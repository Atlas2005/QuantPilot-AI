...
def load_bull_prototype_experiment_harness_outputs(output_dir: str) -> dict[str, object]:
    base = Path(output_dir)
    report_path = base / "bull_prototype_experiment_harness_report.md"
    return {
        "registry": pd.read_csv(base / "bull_prototype_registry.csv")
        if (base / "bull_prototype_registry.csv").exists()
        else pd.DataFrame(),
        "dry_run_plan": pd.read_csv(base / "bull_prototype_dry_run_plan.csv")
        if (base / "bull_prototype_dry_run_plan.csv").exists()
        else pd.DataFrame(),
        "config_validation": pd.read_csv(base / "bull_prototype_config_validation.csv")
        if (base / "bull_prototype_config_validation.csv").exists()
        else pd.DataFrame(),
        "baseline_requirements": pd.read_csv(base / "bull_prototype_baseline_requirements.csv")
        if (base / "bull_prototype_baseline_requirements.csv").exists()
        else pd.DataFrame(),
        "metric_contract": pd.read_csv(base / "bull_prototype_metric_contract.csv")
        if (base / "bull_prototype_metric_contract.csv").exists()
        else pd.DataFrame(),
        "execution_guardrails": pd.read_csv(base / "bull_prototype_execution_guardrails.csv")
        if (base / "bull_prototype_execution_guardrails.csv").exists()
        else pd.DataFrame(),
        "not_executed": pd.read_csv(base / "bull_prototype_not_executed_log.csv")
        if (base / "bull_prototype_not_executed_log.csv").exists()
        else pd.DataFrame(),
        "markdown_report": report_path.read_text(encoding="utf-8")
        if report_path.exists()
        else "",
...
def load_bull_prototype_result_review_outputs(output_dir: str) -> dict[str, object]:
    base = Path(output_dir)
    report_path = base / "bull_prototype_result_review_report.md"
    return {
        "review_summary": pd.read_csv(base / "bull_prototype_review_summary.csv")
        if (base / "bull_prototype_review_summary.csv").exists()
        else pd.DataFrame(),
        "candidate_selection": pd.read_csv(base / "bull_candidate_selection.csv")
        if (base / "bull_candidate_selection.csv").exists()
        else pd.DataFrame(),
        "unresolved_blockers": pd.read_csv(base / "bull_unresolved_blockers.csv")
        if (base / "bull_unresolved_blockers.csv").exists()
        else pd.DataFrame(),
        "v4_closure_status": pd.read_csv(base / "bull_v4_closure_status.csv")
        if (base / "bull_v4_closure_status.csv").exists()
        else pd.DataFrame(),
        "transition_to_v5": pd.read_csv(base / "bull_transition_to_v5_recommendation.csv")
        if (base / "bull_transition_to_v5_recommendation.csv").exists()
        else pd.DataFrame(),
        "guardrails": pd.read_csv(base / "bull_result_review_guardrails.csv")
        if (base / "bull_result_review_guardrails.csv").exists()
        else pd.DataFrame(),
        "limitations": pd.read_csv(base / "bull_result_review_limitations.csv")
        if (base / "bull_result_review_limitations.csv").exists()
        else pd.DataFrame(),
        "markdown_report": report_path.read_text(encoding="utf-8")
        if report_path.exists()
        else "",
...
def load_cross_step_dependency_validator_outputs(output_dir: str) -> dict[str, object]:
    base = Path(output_dir)
    report_path = base / "cross_step_dependency_report.md"
    return {
        "summary": pd.read_csv(base / "cross_step_dependency_summary.csv")
        if (base / "cross_step_dependency_summary.csv").exists()
        else pd.DataFrame(),
        "results": pd.read_csv(base / "cross_step_dependency_results.csv")
        if (base / "cross_step_dependency_results.csv").exists()
        else pd.DataFrame(),
        "guardrails": pd.read_csv(base / "cross_step_dependency_guardrails.csv")
        if (base / "cross_step_dependency_guardrails.csv").exists()
        else pd.DataFrame(),
        "markdown_report": report_path.read_text(encoding="utf-8")
        if report_path.exists()
        else "",
...
def render_bull_prototype_experiment_harness_tab() -> None:
    st.write(
        "Load Step 41 bull prototype experiment harness dry-run outputs."
    )
    st.warning(
        "This panel is educational research diagnostics only. It does not "
        "execute prototype backtests or make any trading-ready claim."
    )
    output_dir = st.text_input(
        "Bull prototype harness output directory",
        value="outputs/bull_prototype_experiment_harness_real_v1",
        key="bull_prototype_experiment_harness_output_dir",
    )
    if not st.button(
        "Load bull prototype harness",
        key="load_bull_prototype_experiment_harness_button",
        type="primary",
    ):
        return

    output = load_bull_prototype_experiment_harness_outputs(output_dir)
    st.subheader("Prototype Registry")
    st.dataframe(output["registry"], width="stretch")
    st.subheader("Dry-Run Plan")
    st.dataframe(output["dry_run_plan"], width="stretch")
    st.subheader("Config Validation")
    st.dataframe(output["config_validation"], width="stretch")
    st.subheader("Baseline Requirements")
    st.dataframe(output["baseline_requirements"], width="stretch")
    st.subheader("Metric Contract")
    st.dataframe(output["metric_contract"], width="stretch")
    st.subheader("Execution Guardrails")
    st.dataframe(output["execution_guardrails"], width="stretch")
    st.subheader("Not Executed Log")
    st.dataframe(output["not_executed"], width="stretch")
    st.subheader("Markdown Report")
    if output["markdown_report"]:
        st.markdown(output["markdown_report"])
        st.download_button(
            "Download bull prototype harness report",
            data=output["markdown_report"],
            file_name="bull_prototype_experiment_harness_report.md",
            mime="text/markdown",
            key="download_bull_prototype_experiment_harness_report_button",
        )
    else:
        st.info("No Markdown report text is available.")
...
def render_bull_prototype_result_review_tab() -> None:
    st.write(
        "Load Step 43 bull prototype result review and V4 closure outputs."
    )
    st.warning(
        "This panel is educational research diagnostics only. It is a review "
        "and closure view, not a trading-ready claim."
    )
    output_dir = st.text_input(
        "Bull prototype result review output directory",
        value="outputs/bull_prototype_result_review_real_v1",
        key="bull_prototype_result_review_output_dir",
    )
    if not st.button(
        "Load bull prototype result review",
        key="load_bull_prototype_result_review_button",
        type="primary",
    ):
        return

    output = load_bull_prototype_result_review_outputs(output_dir)
    st.subheader("Prototype Review Summary")
    st.dataframe(output["review_summary"], width="stretch")
    st.subheader("Candidate Selection")
    st.dataframe(output["candidate_selection"], width="stretch")
    st.subheader("Unresolved Bull Blockers")
    st.dataframe(output["unresolved_blockers"], width="stretch")
    st.subheader("V4 Closure Status")
    st.dataframe(output["v4_closure_status"], width="stretch")
    st.subheader("Transition to V5 Recommendation")
    st.dataframe(output["transition_to_v5"], width="stretch")
    st.subheader("Guardrails")
    st.dataframe(output["guardrails"], width="stretch")
    st.subheader("Limitations")
    st.dataframe(output["limitations"], width="stretch")
    ...
    if output["markdown_report"]:
        st.markdown(output["markdown_report"])
        st.download_button(
            "Download bull prototype result review report",
            data=output["markdown_report"],
            file_name="bull_prototype_result_review_report.md",
            mime="text/markdown",
            key="download_bull_prototype_result_review_report_button",
        )
    else:
        st.info("No Markdown report text is available.")
...
def render_cross_step_dependency_validator_tab() -> None:
    st.write(
        "Load V6 Step 3 research-only cross-step dependency validation outputs."
    )
    st.warning(
        "This panel reads the V6 Step 3 dependency validation report only. It "
        "does not run backtests, fetch market data, train models, change "
        "thresholds, connect to brokers, submit orders, or make any "
        "trading-ready claim."
    )
    output_dir = st.text_input(
        "Cross-step dependency validator directory",
        value="outputs/cross_step_dependency_validator_real_v1",
        key="cross_step_dependency_validator_output_dir",
    )
    if not st.button(
        "Load cross-step dependency validation",
        key="load_cross_step_dependency_validator_button",
        type="primary",
    ):
        return

    output = load_cross_step_dependency_validator_outputs(output_dir)
    ...
    if output["markdown_report"]:
        st.markdown(output["markdown_report"])
        st.download_button(
            "Download dependency validation report",
            data=output["markdown_report"],
            file_name="cross_step_dependency_report.md",
            mime="text/markdown",
            key="download_cross_step_dependency_validator_report_button",
        )
    else:
        st.info("No Markdown report text is available.")
