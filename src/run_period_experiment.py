...
DEFAULT_PERIODS = "2021,2022,2023,2024"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Test risk-control settings across symbols and year periods."
    )
    parser.add_argument(
        "--symbols",
    ...
    parser.add_argument(
        ...
        help="Comma-separated years, for example 2021,2022,2023,2024.",
    )
    parser.add_argument(
        "--adjust",
        default="qfq",
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="Optional CSV path for saving raw period experiment results.",
    )
    parser.add_argument(
        "--compact",
        action="store_true",
...
def make_error_row(period: str, symbol: str, error: str) -> dict:
...
def run_symbol_period(args: argparse.Namespace, period: str, symbol: str) -> list[dict]:
    start_date, end_date = period_to_dates(period)
    print(f"Fetching data once for period {period}, symbol {symbol}...")
    ...
    print(f"Fetched {len(stock_data)} rows for {period} {symbol}.")
    print(f"Preparing indicators and strategy signals once for {period} {symbol}.")
    prepared_data = add_all_indicators(stock_data)
    prepared_data = generate_ma_crossover_signals(prepared_data)

    return [
        run_scenario(period, symbol, prepared_data, args.initial_cash, scenario, args)
...
def build_scenario_ranking(overall_df: pd.DataFrame) -> pd.DataFrame:
    if overall_df.empty:
        return pd.DataFrame(
            columns=[
                "rank",
...
def print_period_results(results_df: pd.DataFrame) -> None:
    print("Period Experiment Results")
    print("-------------------------")
    print(format_results_table(results_df).to_string(index=False))


def print_compact_results(results_df: pd.DataFrame) -> None:
    print("Compact Period Results")
    print("----------------------")
...
def print_period_summary(summary_df: pd.DataFrame) -> None:
    print()
    print("Scenario Period Summary")
    print("-----------------------")

    if summary_df.empty:
        print("No successful period scenario results to summarize.")
        return
...
def print_overall_summary(summary_df: pd.DataFrame) -> None:
    print()
    print("Overall Scenario Summary")
    print("------------------------")
...
def main() -> None:
    ...
    if not symbols:
        ...
        sys.exit(1)
    if not periods:
        print("Error: please provide at least one period year.")
        sys.exit(1)

    print("QuantPilot-AI Period Experiment")
    print("-------------------------------")
    print(f"Selected symbols: {', '.join(symbols)}")
    print(f"Selected periods: {', '.join(periods)}")
    ...
    rows = []
    fetch_jobs = [(period, symbol) for period in periods for symbol in symbols]
    for index, (period, symbol) in enumerate(fetch_jobs):
        try:
            rows.extend(run_symbol_period(args, period, symbol))
        except Exception as exc:
            print(f"Error: failed to process {period} {symbol}: {exc}")
            rows.append(make_error_row(period, symbol, str(exc)))

        if index < len(fetch_jobs) - 1:
            print("Waiting 1 second before the next symbol-period fetch...")
            print()
            time.sleep(1)

    results_df = pd.DataFrame(rows, columns=RESULT_COLUMNS)
    period_summary_df = build_period_summary(results_df)
    overall_summary_df = build_overall_summary(results_df)
    ranking_df = build_scenario_ranking(overall_summary_df)

    print()
    if args.compact:
        print_compact_results(results_df)
    else:
        print_period_results(results_df)

    print_period_summary(period_summary_df)
    print_overall_summary(overall_summary_df)
    ...
    if args.output:
        ...
        print()
        print(f"Saved raw period experiment results to {args.output}")
