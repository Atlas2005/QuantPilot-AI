...
try:
    from .candidate_stress_test import parse_symbols, save_candidate_stress_test
except ImportError:
    from candidate_stress_test import parse_symbols, save_candidate_stress_test
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run candidate stress test and market regime validation.",
    )
...
def main() -> None:
    ...
    try:
        ...
        print(f"Error: candidate stress test failed: {exc}")
        sys.exit(1)

    print("QuantPilot-AI Candidate Stress Test")
    print("-----------------------------------")
    print(f"Factor directory: {args.factor_dir}")
    print(f"Symbols: {symbols}")
    print(f"Recommendations: {args.recommendations}")
    print(f"Output directory: {args.output_dir}")
    print(f"Regime window: {args.regime_window}")
    print(f"Walk-forward candidate enabled: {args.enable_walk_forward}")
    print()
    print("Output Files")
    print("------------")
