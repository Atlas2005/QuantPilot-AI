...
## V4 Step 38: Bull Trade/Window Diagnostics

Step 38 enhances diagnostics output for the already-selected Step 34 bull
threshold, `0.65 / 0.50`, for `canonical_reduced_40` with
`logistic_regression`. It exports trade-level, signal timeline, and fixed-window
diagnostics when they can be reconstructed from the existing pipeline.

Example:

```powershell
python src/run_bull_trade_window_diagnostics.py --bull-dir outputs/bull_regime_threshold_remediation_real_v1 --drilldown-dir outputs/bull_regime_failure_drilldown_real_v1 --output-dir outputs/bull_trade_window_diagnostics_real_v1
```

This step is diagnostic-only. It does not tune thresholds, retrain models,
change factors, add data sources, or add agents. It does not upgrade any
candidate to trading-ready; `canonical_reduced_40` remains research-only.
...
## V4 Step 41: Bull Prototype Experiment Harness

Step 41 creates controlled harness infrastructure for future bull prototype
experiments. It registers Step 40 prototypes, validates dry-run configs, defines
baseline requirements and metric contracts, and records execution guardrails.
It does not execute prototypes, run real prototype backtests, retrain models,
change thresholds, change factors, add data sources, or add agents.

Example:

```powershell
python src/run_bull_prototype_experiment_harness.py --prototype-design-dir outputs/bull_remediation_prototype_design_real_v1 --diagnostics-dir outputs/bull_trade_window_diagnostics_real_v1 --integrated-dir outputs/integrated_remediation_revalidation_real_v1 --output-dir outputs/bull_prototype_experiment_harness_real_v1
```

No trading-ready upgrade is made. `canonical_reduced_40` remains research-only
and bull remediation remains failed until future controlled execution and
validation explicitly show otherwise.
...
V5 Step 4 adds a research-only exit planning layer that reads the V5 Step 3
sized positions output and writes explicit static exit rules for each sized
position.

Example:

```powershell
python src/run_exit_engine.py --input-path outputs/position_sizing_engine_real_v1/sized_positions.csv --output-dir outputs/exit_engine_real_v1
```

By default, the runner reads
`outputs/position_sizing_engine_real_v1/sized_positions.csv`, applies a `0.05`
stop loss, a `0.10` take profit, a 10-day maximum holding period, and the
`exit_if_underperform_benchmark_by_3pct_after_5_days` benchmark lag rule. This
step does not run backtests, tune thresholds, retrain models, change features,
add data sources, connect to a broker, perform live trading, execute orders, or
change trading-readiness status. The project remains educational/research-only
and not trading-ready.
...
V5 Step 6 adds a deterministic research-only paper trading ledger that reads the
V5 Step 5 daily trading plan outputs and creates simulated paper orders, fills,
positions, cash ledger rows, and a combined paper trade ledger.

Example:

```powershell
python src/run_paper_trading_ledger.py --output-dir outputs/paper_trading_ledger_real_v1
```

By default, the runner reads `outputs/daily_trading_plan_real_v1`. Rows marked
`sized_position` become simulated paper BUY fills at their entry price, deferred
rows are recorded as not filled, and exit-plan rows are attached as exit rules
only. This step does not fetch live market data, execute exits, submit real
orders, connect to a broker, run backtests, tune thresholds, retrain models,
change features, add data sources, or make strategy performance claims. The
project remains educational/research-only and not trading-ready. The recommended
...
V6 Step 1 starts `validation_and_simulation_hardening` by recording the current
V5 output state as a stable research baseline. It inspects existing local V5
Step 1-10 output directories and records file counts, byte sizes, key summary
files, key guardrail files, key reports, and detectable safety flag true counts.

Example:

```powershell
python src/run_validation_baseline_manifest.py --output-dir outputs/validation_baseline_manifest_real_v1
```

Generated files include `validation_baseline_summary.csv`,
`validation_baseline_manifest.csv`, `validation_baseline_guardrails.csv`,
`validation_baseline_report.md`, and `run_config.json`. This step does not run
backtests, fetch market data, change thresholds, retrain models, change
features, add data sources, request credentials, import broker SDKs, connect to
brokers, create order execution paths, submit orders, perform live trading, or
mark anything trading-ready. The expected baseline status is
...
V6 Step 4 reruns selected deterministic local-only V5/V6 output-producing
commands into an isolated `rerun_workspace` under the Step 4 output directory,
then compares the rerun outputs against canonical outputs. It checks expected
files, row counts, key summary values, normalized fingerprints, and forbidden
safety flags.

Example:

```powershell
python src/run_reproducibility_rerun_validator.py --output-dir outputs/reproducibility_rerun_validator_real_v1
```

Generated files include `reproducibility_rerun_summary.csv`,
`reproducibility_rerun_results.csv`, `reproducibility_rerun_guardrails.csv`,
`reproducibility_rerun_report.md`, and `run_config.json`. Rerun artifacts are
written below `outputs/reproducibility_rerun_validator_real_v1/rerun_workspace/`.
This step does not run backtests, fetch market data, change thresholds, retrain
models, change features, add data sources, request credentials, import broker
SDKs, connect to brokers, execute or submit orders, perform live trading,
